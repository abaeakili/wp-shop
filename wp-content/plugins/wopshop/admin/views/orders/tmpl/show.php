...
$print=$this->print;
?>
<div class="wopshop_edit">
<form action="admin.php?option=orders" method="post" name="adminForm" id="adminForm">
...
<tr>
  <td width="50%" style="vertical-align:top;padding-top:14px;">
    ...
    <thead>    
      <tr>
        <th colspan="2">
          <?php echo _WOP_SHOP_ORDER_PURCHASE;?>
        </th>
      </tr>
     </thead> 
      <tr>
        <td width="50%">
          <b><?php echo _WOP_SHOP_NUMBER;?></b>
        </td>
        <td>
      ...
      <tr>
        <td>
          <b><?php echo _WOP_SHOP_IPADRESS;?></b>
       </td>
        <td>
  ...
  <td width="50%" style="vertical-align: top">
    <div class="tabs">
        <ul class="tab-links">
            <li class="active"><a href="#first-page" data-toggle="tab"><?php echo _WOP_SHOP_STATUS_CHANGE;?></a></li>
            <li><a href="#second-page" data-toggle="tab"><?php echo _WOP_SHOP_ORDER_HISTORY;?></a></li>
        </ul>
                ...
                <table width="100%">
                  <tr>
                    <th colspan="2" align="center">
                      <?php echo _WOP_SHOP_STATUS_CHANGE?>:
                    </th>
                  </tr>
                  <tr>
                    <td colspan="2">
                      <?php echo _WOP_SHOP_ORDER_STATUS?>
                      <?php echo $lists['status'];?>
                      <input type="button" class="button" name="update_status" onclick="verifyStatus(<?php echo $order->order_status?>, <?php echo $order->order_id?>, '<?php echo _WOP_SHOP_CHANGE_ORDER_STATUS;?>', 1)" value="<?php echo _WOP_SHOP_UPDATE_STATUS?>" />
                    </td>
                  </tr>
                  <tr>
                    <td colspan="2">
                      <table>
                        <tr>
                          <td>
                            <?php echo _WOP_SHOP_COMMENT;?>:
                          </td>
                          <td>
                            <textarea id="comments" name="comments"></textarea>
                          </td>
                          <td>
                            <input type="checkbox" class="inputbox"  name="notify" id="notify" /><label for="notify">  <?php echo _WOP_SHOP_NOTIFY_USER;?></label><br />
                            <input type="checkbox" class="inputbox"  name="include" id="include" /><label for="include">  <?php echo _WOP_SHOP_INCLUDE_COMMENT;?></label>
                          </td>
                        </tr>
                      </table>
                    </td>
        ...
        <table>
            <tr class="bold">
              <td>
                <?php echo _WOP_SHOP_DATE_ADDED;?>
              </td>
              <td>
                <?php echo _WOP_SHOP_NOTIFY_CUSTOMER;?>
              </td>
              <td>
          ...
          <?php foreach($order_history as $history) {?>
            <tr>
              ...
              <td class="center">
                ...
                <img src="<?php echo WOPSHOP_PLUGIN_URL; ?>assets/images/<?php echo $notify_customer?>" alt="notify_customer" border="0" />
              </td>
              <td>
...
<tr>
 <td>
   <a target="_blank" href="admin.php?page=products&task=edit&product_id=<?php print $item->product_id?>">
...
<tr>
 <td colspan="5" style="height: 20px">
    ...
    <div style="text-align:right;">
        <i><?php print _WOP_SHOP_WEIGHT_PRODUCTS?>: <span><?php print formatweight($this->order->weight);?></span></i>
    </div><br/>
  <?php }?>
 </td>
</tr>
...
<tr>
    ...
    <?php } ?>
    <th width="34%">
    <?php echo _WOP_SHOP_CUSTOMER_COMMENT?>
    </th>
</tr>
</thead>
<tr>
    ...
    <td valign="top">
        ...
        <div><?php echo _WOP_SHOP_DELIVERY_TIME.": ".$order->delivery_time_name?></div>
        <?php }?>
        <?php if ($order->delivery_date_f){?>
        <div><?php echo _WOP_SHOP_DELIVERY_DATE.": ".$order->delivery_date_f?></div>
        <?php }?>
    </td>
    <?php } ?>
    <?php if (!$this->config->without_payment){?>
...
<tr>
    <th width="50%">
        <?php echo _WOP_SHOP_FILE_SALE?>
    </th>
    <th>
        <?php echo _WOP_SHOP_COUNT_DOWNLOAD?>
    </th>
</tr>
</thead>
...
<div class="order_stat_file_download_clear">
    <a onclick="return confirm('<?php print _WOP_SHOP_CLEAR?>')" href="admin.php?page=orders&task=stat_file_download_clear&order_id=<?php print $order->order_id?>"><?php print _WOP_SHOP_CLEAR?></a>
</div>
<?php }?>
