...
<table width="794px" align="center" border="0" cellspacing="0" cellpadding="0" style="line-height:100%;">
  ...
  <tr class = "bg_gray">
     <td colspan = "2">
        <h3><?php print _WOP_SHOP_EMAIL_PURCHASE_ORDER?></h3>
     </td>
  </tr>
  <tr><td style="height:10px;font-size:1px;">&nbsp;</td></tr>
  <tr>
     <td width="50%">
        <?php print _WOP_SHOP_ORDER_NUMBER?>:
     </td>
     <td width="50%">
  ...
  <tr>
     <td>
        <?php print _WOP_SHOP_ORDER_DATE?>:
     </td>
     <td>
  ...
  <tr>
     <td>
        <?php print _WOP_SHOP_ORDER_STATUS?>:
     </td>
     <td>
...
<?php if ($this->show_customer_info){?>
  ...
  <tr class="bg_gray">
    <td colspan="2" width = "50%">
       <h3><?php print _WOP_SHOP_CUSTOMER_INFORMATION?></h3>
    </td>
  </tr>
  <tr>
    <td  style="vertical-align:top;padding-top:10px;" width = "50%">
      <table cellspacing="0" cellpadding="0" style="line-height:100%;">
        ...
        <tr>
          <td><?php print _WOP_SHOP_FIRMA_CODE?>:</td>
        ...
        <tr>
          <td><?php print _WOP_SHOP_VAT_NUMBER?>:</td>
    ...
    <table cellspacing="0" cellpadding="0" style="line-height:100%;">
        <tr>
            <td colspan=2><b><?php print _WOP_SHOP_EMAIL_SHIP_TO?></b></td>
        </tr>
...
<?php }?>
  ...
  <tr>
    <td colspan = "2" class="bg_gray">
      <h3><?php print _WOP_SHOP_ORDER_ITEMS ?></h3>
    </td>
  </tr>
  <tr>
    <td colspan="2" style="padding:0px;padding-top:10px;">
       <table width="100%" cellspacing="0" cellpadding="0" class="table_items">
        <tr><td colspan="5" style="vertical-align:top;padding-bottom:5px;font-size:1px;"><div style="height:1px;border-top:1px solid #999;"></div></td></tr>
         <tr class = "bold">            
            <td width="45%" style="padding-left:10px;padding-bottom:5px;"><?php print _WOP_SHOP_NAME_PRODUCT?></td>            
            <td width="15%" style="padding-bottom:5px;"><?php if ($this->config->show_product_code_in_order){?><?php print _WOP_SHOP_EAN_PRODUCT?><?php } ?></td>
            <td width="10%" style="padding-bottom:5px;"><?php print _WOP_SHOP_QUANTITY?></td>
            <td width="15%" style="padding-bottom:5px;"><?php print _WOP_SHOP_SINGLEPRICE?></td>
            <td width="15%" style="padding-bottom:5px;"><?php print _WOP_SHOP_PRICE_TOTAL?></td>
         </tr>
         ...
         <tr class="vertical">
           <td>
                ...
                <div class="deliverytime"><?php print _WOP_SHOP_DELIVERY_TIME?>: <?php print $prod->delivery_time?></div>
                <?php }?>
           </td>           
           ...
           <td>
				...
				<?php if ($this->config->cart_basic_price_show && $prod->basicprice>0){?>
                    <div class="basic_price"><?php print _WOP_SHOP_BASIC_PRICE?>: <span><?php print sprintBasicPrice($prod);?></span></div>
                <?php }?>
           </td>
           <td>
         ...
         <tr>
            ...
            <?php foreach($files as $file){?>
                <div><?php print $file->file_descr?> <a href="<?php print SEFLink('controller=product&task=getfile&oid='.$this->order->order_id.'&id='.$file->id.'&hash='.$this->order->file_hash.'&rl=1')?>"><?php print _WOP_SHOP_DOWNLOAD?></a></div>
            <?php }?>    
            </td>
         </tr>
         <?php }?>
         ...
         <tr>
            <td colspan="5" style="text-align:right;font-size:11px;">            
                <?php print _WOP_SHOP_WEIGHT_PRODUCTS?>: <span><?php print formatweight($this->order->weight);?></span>
            </td>
         </tr>   
         <?php }?>
      <?php if ($this->show_total_info){?>
         ...
         <tr>
           <td colspan="4" align="right" style="padding-right:15px;"><?php print _WOP_SHOP_SUBTOTAL ?>:</td>
         ...
         <tr>
           <td colspan="4" align="right" style="padding-right:15px;"><?php print _WOP_SHOP_RABATT_VALUE ?>: </td>
         ...
         <tr>
           <td colspan="4" align="right" style="padding-right:15px;"><?php print _WOP_SHOP_SHIPPING_PRICE ?>:</td>
         ...
         <tr>
           <td colspan="4" align="right" style="padding-right:15px;"><?php print _WOP_SHOP_PACKAGE_PRICE?>:</td>
         ...
         <?php if (!$this->client){?>
         <tr>
           <td colspan="5" class="bg_gray"><?php print _WOP_SHOP_CUSTOMER_NOTE ?></td>
         </tr>
         <tr>
...
<?php if ($this->show_payment_shipping_info){?>
  ...
  <tr class = "bg_gray">
    ...
    <td>
        <h3><?php print _WOP_SHOP_PAYMENT_INFORMATION ?></h3>
    </td>    
    <?php }?>
    <td <?php if ($this->config->without_payment){?> colspan="2" <?php }?>>
        <?php if (!$this->config->without_shipping){?>
        <h3><?php print _WOP_SHOP_SHIPPING_INFORMATION ?></h3>
        <?php } ?>
    </td>    
  </tr>
    ...
    <td valign="top" <?php if ($this->config->without_payment){?> colspan="2" <?php }?>>
        <?php if (!$this->config->without_shipping){?>
            ...
            <?php if ($this->config->show_delivery_time_checkout && $this->order->order_delivery_time){
                print "<div>"._WOP_SHOP_ORDER_DELIVERY_TIME.": ".$this->order->order_delivery_time."</div>";
            }            
            if ($this->config->show_delivery_date && $order->delivery_date_f){
                print "<div>"._WOP_SHOP_DELIVERY_DATE.": ".$order->delivery_date_f."</div>";
            }
        }
        ?>
    </td>  
...
<?php }?>
  ...
  <tr>
    <td colspan="2"><br/><br/><a class = "policy" target="_blank" href="<?php print SEFLink('controller=content&task=view&page=return_policy&order_id='.$this->order->order_id, 1);?>"><?php print _WOP_SHOP_RETURN_POLICY?></a></td>
  </tr>
  <?php }?>
  <?php if ($this->client){?>
  <tr>
