...
<div class="wshop myorderinfo">
    ...
    <?php if ($this->config->order_send_pdf_client) : ?>
        <div class="downlod_order_invoice">
            <a class="btn" target="_blank" href="<?php print $this->config->pdf_orders_live_path."/".$order->pdf_file;?>">
                <?php print _WOP_SHOP_DOWNLOAD_INVOICE?>
            </a>
        </div>
    <?php endif; ?>
    ...
    <div class = "row-fluid order_total">
        <div class = "span12">
            <b><?php print _WOP_SHOP_PRICE_TOTAL ?>:</b>
    ...
    <div class = "row-fluid userinfo">
        <div class = "span6 userbillinfo">
            <table class = "wshop">
                ...
                <tr>
                    <td><?php print _WOP_SHOP_FIRMA_NAME?>:</td>
                    <td><?php print $this->order->firma_name?></td>
                </tr>
                <?php } ?>
                <?php if ($this->config_fields['f_name']['display']){?>
                <tr>
                    <td width = "40%"><?php print _WOP_SHOP_FULL_NAME?>:</td>
    ...
    <div class="product_head">
        <strong><?php echo _WOP_SHOP_PRODUCTS; ?></strong>
    </div>
        ...
        <tr>
            ...
            <?php if ($this->config->show_product_code_in_order){?>
                <th class="product_code" width = "15%">
                    <?php print _WOP_SHOP_EAN_PRODUCT?>
                </th>
            <?php }?>
            ...
            <tr class="wshop_prod_cart <?php if ($i % 2 == 0) print "even"; else print "odd"?>">
                <td class="product_name">
                    ...
                    <div class="data">
                        ...
                        <div class="attribs">
                            <?php 
                            //print sprintAtributeInOrder($prod->product_attributes).sprintFreeAtributeInOrder($prod->product_freeattributes);
                            $r= sprintAtributeInOrder($prod->product_attributes).sprintFreeAtributeInOrder($prod->product_freeattributes);
                            $r = str_replace('\n', '<br>',$r);
                            echo $r;
                            print $prod->_ext_attribute_html;?>
                        </div>
                        <?php if (count($files)){?>
                            <div class="filelist">
                                <?php foreach($files as $file){?>
                                    <div class="file">
										...
										<a class="download" href="<?php print SEFLink('controller=product&task=getfile&oid='.$this->order->order_id.'&id='.$file->id.'&hash='.$this->order->file_hash); ?>">											
                                            <?php print _WOP_SHOP_DOWNLOAD?>
                                        </a>
                                    </div>
                                <?php }?>
                            </div>
    ...
    <?php if ($this->config->show_weight_order){?>  
        <div class="weightorder">
            <?php print _WOP_SHOP_WEIGHT_PRODUCTS?>: 
    ...
    <?php if (!$this->config->without_shipping){?>
        <div class="shipping_block_info">
            <div class="shipping_head">
                <b><?php print _WOP_SHOP_SHIPPING_INFORMATION ?></b>
            </div>
            ...
            <?php if ($order->delivery_time_name){?>
                <div class="delivery_time">
                    <?php echo _WOP_SHOP_DELIVERY_TIME.": ".$order->delivery_time_name?>
                </div>
            <?php }?>
            
            <?php if ($order->delivery_date_f){?>
                <div class="delivery_date">
                    <?php echo _WOP_SHOP_DELIVERY_DATE.": ".$order->delivery_date_f?>
                </div>
            <?php }?>
        </div>
    <?php }?>
    ...
    <?php if (!$this->config->without_payment){?>
        <div class="payment_block_info">
            <div class="payment_head">
                <b><?php print _WOP_SHOP_PAYMENT_INFORMATION ?></b>
            </div>
    ...
    <?php if ($order->order_add_info){ ?>
        <div class="order_comment">
            <div class="order_comment_head">
                <b><?php print _WOP_SHOP_ORDER_COMMENT ?></b>
            </div>
    ...
    <div class="history">
        <div class="history_head">
            <b><?php print _WOP_SHOP_ORDER_HISTORY ?></b>
        </div>
    ...
    <?php if ($this->allow_cancel){?>
        <div class="button_cancel">
            <a href="<?php print SEFLink('controller=user&task=cancelorder&order_id='.$order->order_id)?>" class = "btn">
                <?php print _WOP_SHOP_CANCEL_ORDER?>
            </a>
        </div>
    <?php }?>
