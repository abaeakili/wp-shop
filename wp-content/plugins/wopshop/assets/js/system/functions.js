...
function showHideReview(){
	jQuery('#wshop_review_write').show();
}
    ...
    prevAjaxHandler = jQuery.getJSON(
        ...
        function(json){
            ...
            if (json.images && json.images.length>0){
                ...
                var html_middle_img = "";
                var html_zoom_img = '';
                if (typeof(wshop_product_hide_zoom_image)==='undefined') wshop_product_hide_zoom_image = 0;
                if (!wshop_product_hide_zoom_image){
                    html_zoom_img = ' <div class="text_zoom"><img alt="zoom" src="'+liveimgpath+'/search.png" /> '+translate_zoom_image+'</div>';
                }
                for(var j=0;j<count_prod_img;j++){
                    html_thumb_img+='<img class="wshop_img_thumb" src="'+liveproductimgpath+'/thumb_'+json.images[j]+'" onclick = "showImage('+j+')" /> ';
...
function reloadAttrValue(){    
    for(var id in attr_value){
        if (jQuery("input[name=wshop_attr_id\\["+id+"\\]]").attr("type")=="radio"){
            attr_value[id] = jQuery("input[name=wshop_attr_id\\["+id+"\\]]:checked").val();
        }else{
            attr_value[id] = jQuery("#wshop_attr_id"+id).val();
        }
    }
}
