...
function modal_insert_product_to_order_javascript(){
    jQuery(document).on('click', 'a.modalInsertProduct', function(){
        ...
        jQuery('<div id="my-dialog">Loading...</div>').dialog({
            ...
            height: '600',
            open: function() {
                jQuery('#my-dialog').html('<iframe width="100%" height="600" frameborder="0" src="admin-ajax.php?page=products&tab=productlistselectable&action=modal_insert_product_to_order&e_name='+i+'"></iframe>');
            },
            close: function() {
...
function deleteFotoManufacturer(manufacturer_id){
    ...
    jQuery.ajax({
        type: "POST",
        url: 'admin.php?page=options&tab=manufacturers&task=deleteFoto',
	data: data,
	dataType: 'html',
...
function deleteFotoProductlabel(productlabel_id){
    ...
    jQuery.ajax({
        type: "POST",
        url: 'admin.php?page=options&tab=productlabels&task=deleteFoto',
	data: data,
	dataType: 'html',
	error: function(jqXHR, exception) {if (jqXHR.status === 0) {alert('Not connect.\n Verify Network.');} else if (jqXHR.status == 404) {alert('Requested page not found. [404]');} else if (jqXHR.status == 500) {alert('Internal Server Error [500].');} else if (exception === 'parsererror') {alert('Requested JSON parse failed.');} else if (exception === 'timeout') {alert('Time out error.');} else if (exception === 'abort') {alert('Ajax request aborted.');} else {jQuery(".message-process").html('Query Error');alert('Uncaught Error.\n' + jqXHR.responseText);}},
	beforeSend:function() {},
...
function deleteFotoCategory(catid){
    var url = 'admin.php?page=categories&task=delete_foto&catid='+catid;
...
function deleteFotoProduct(id){
    var url = 'admin-ajax.php?page=products&task=delete_foto&action=product_delete_foto&id='+id;
    //var url = 'admin.php?page=products&task=delete_foto&id='+id;
    function showResponse(data){
...
function changeCategory(){
    ...
    var url = 'admin-ajax.php?page=categories&task=sorting_cats_html&action=category_parent_sorting&catid='+catid;
    function showResponse(data){
...
function verifyStatus(orderStatus, orderId, message, extended, limit){
   if (extended == 0){
       ...
       if (statusNewId == orderStatus){
         ...
         location.href = 'admin.php?page=orders&task=update_status&js_nolang=1&order_id=' + orderId + '&order_status=' + statusNewId + limit + isChecked;
       }
   } else {
       ...
       if (statusNewId == orderStatus){
         ...
         location.href = 'admin.php?page=orders&task=&task=update_one_status&js_nolang=1&order_id=' + orderId + '&order_status=' + statusNewId + isChecked + includeComment + '&comments=' + encodeURIComponent($F_('comments'));
       }
   }
}
...
function loadProductInfoRowOrderItem(pid, num, currency_id){
    var url = 'admin-ajax.php?page=products&task=loadproductinfo&action=modal_insert_product_to_order_json&order_id=&product_id='+pid+'&currency_id='+currency_id;
...
function deleteFotoAttribValue(id){
    var url = 'admin.php?page=products&tab=attributesvalues&task=delete_foto&id='+id;
    function showResponse(data){
...
function reloadProductExtraField(product_id){
    ...
    //var url = 'admin.php?option=com_jshopping&controller=products&task=product_extra_fields&product_id='+product_id+catsurl+"&ajax=1";
    var url = 'admin-ajax.php?page=products&task=product_extra_fields&action=product_cat_attr&product_id='+product_id+catsurl;
    function showResponse(data){
...
function deleteFileProduct(id, type){
    var url = 'admin-ajax.php?page=products&task=delete_file&action=product_delete_file&id='+id+"&type="+type;
    function showResponse(data){
        if (type=="demo"){
...
function releted_product_search(start, no_id){
    ...
    var url = 'admin-ajax.php?page=products&task=search_related&action=search_related&&start='+start+'&no_id='+no_id+'&text='+encodeURIComponent(text);
...
function deleteVideoProduct(id){
    var url = 'admin-ajax.php?page=products&task=delete_video&action=delete_video&id='+id;
    function showResponse(data){
...
function addFieldShPrice(){
    ...
    html += '<td style="text-align:center"><a class="btn btn-micro" href="#" onclick="delete_shipping_weight_price_row('+shipping_weight_price_num+');return false;"><i class="glyphicon wshop-icon glyphicon-remove-circle"></i></a></td>';
    html += '</tr>';
...
function editAttributeExtendParams(id){
    window.open('admin.php?page=products&task=edit&product_attr_id='+id,'windowae','width=1000, height=760, scrollbars=yes,status=no,toolbar=no,menubar=no,resizable=yes,location=yes');
}