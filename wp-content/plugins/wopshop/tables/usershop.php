	...
	function check($type = null){
        ...
        if ($config_fields['title']['require']){
            if (!intval($this->title)) {
                $this->_error = addslashes(_WOP_SHOP_REGWARN_TITLE);
                return false;
            }
        }
        ...
        if ($config_fields['f_name']['require']){
		    if(trim($this->f_name) == '') {
			    $this->_error = addslashes(_WOP_SHOP_REGWARN_NAME);
			    return false;
		    }
        }
        ...
        if ($config_fields['l_name']['require']){
		    if(trim($this->l_name) == '') {
			    $this->_error = addslashes(_WOP_SHOP_REGWARN_LNAME);
			    return false;
		    }
        }
		...
		if ($config_fields['m_name']['require']){
            if(trim($this->m_name) == '') {
                $this->_error = addslashes(_WOP_SHOP_REGWARN_MNAME);
                return false;
            }
        }
        ...
        if ($config_fields['firma_name']['require']){
            if(trim($this->firma_name) == '') {
                $this->_error = addslashes(_WOP_SHOP_REGWARN_FIRMA_NAME);
                return false;
            }
        }
        ...
        if ($config_fields['client_type']['require']){
            if(trim($this->client_type) == 0) {
                $this->_error = addslashes(_WOP_SHOP_REGWARN_CLIENT_TYPE);
                return false;
            }
        }
        ...
        if ($this->client_type==2 || !$config_fields['client_type']['display']){
            if ($config_fields['firma_code']['require']){
                if(trim($this->firma_code) == '') {
                    $this->_error = addslashes(_WOP_SHOP_REGWARN_FIRMA_CODE);
                    return false;
                }
            }
            ...
            if ($config_fields['tax_number']['require']){
                if(trim($this->tax_number) == '') {
                    $this->_error = addslashes(_WOP_SHOP_REGWARN_TAX_NUMBER);
                    return false;
                }
            }
        }        
		...
		if ($config_fields['email']['require'] || $this->email!=''){
		    if ((trim($this->email=="")) || !is_email($this->email)){
			    $this->_error = addslashes(_WOP_SHOP_REGWARN_MAIL);
			    return false;
		    }
        }
        ...
        if ($config_fields['birthday']['require']){
            if(trim($this->birthday) == '') {
                $this->_error = addslashes(_WOP_SHOP_REGWARN_BIRTHDAY);
                return false;
            }
        }		
		...
		if ($type == "register"){
            if ($config_fields['u_name']['require']){
				if(trim($this->u_name) == '') {
					$this->_error = addslashes(_WOP_SHOP_REGWARN_UNAME);
					return false;
				}
			}
			if ($this->u_name!=''){
				if (preg_match("#[<>\"'%;()&]#i", $this->u_name) || strlen(utf8_decode($this->u_name )) < 2) {
					$this->_error = sprintf(addslashes(_WOP_SHOP_VALID_AZ09),addslashes(_WOP_SHOP_USERNAME),2);
					return false;
				}
        ...
        if ($type=='editaccount'){
            if ($config_fields['password']['require']){
                if(trim($this->password) == '') {
                    $this->_error = addslashes(_WOP_SHOP_REGWARN_PASSWORD);
                    return false;
                }
            }
            if (($this->password || $this->password2) && $config_fields['password_2']['display'] && $this->password!=$this->password2){
		...
		if ($this->email!=''){
			...
			if($xid && $xid != intval($this->id)){
				$this->_error = addslashes(_WOP_SHOP_REGWARN_EMAIL_INUSE);
				return false;
			}
		}
        ...
        if ($config_fields['home']['require']){
            if(trim($this->home) == '') {
                $this->_error = addslashes(_WOP_SHOP_REGWARN_HOME);
                return false;
            }
        }
        ...
        if ($config_fields['apartment']['require']){
            if(trim($this->apartment) == '') {
                $this->_error = addslashes(_WOP_SHOP_REGWARN_APARTMENT);
                return false;
            }
        }
        ...
        if ($config_fields['street']['require']){
		    if(trim($this->street) == '') {
			    $this->_error = addslashes(_WOP_SHOP_REGWARN_STREET);
			    return false;
		    }
        }
        ...
        if ($config_fields['zip']['require']){
		    if (trim($this->zip) == ""){
		        $this->_error = addslashes( _WOP_SHOP_REGWARN_ZIP );
		        return false;
		    }
        }
        ...
        if ($config_fields['city']['require']){
		    if (trim($this->city) == ''){
		        $this->_error = addslashes( _WOP_SHOP_REGWARN_CITY );
		        return false;
		    }
        }		
        ...
        if ($config_fields['state']['require']){
            if (trim($this->state) == ''){
                $this->_error = addslashes( _WOP_SHOP_REGWARN_STATE ); //region
                return false;
            }
        }
        ...
        if ($config_fields['country']['require']){
		    if(!intval($this->country)) {
			    $this->_error = addslashes(_WOP_SHOP_REGWARN_COUNTRY);
			    return false;
		    }
        }		
        ...
        if ($config_fields['phone']['require']){	
		    if(trim($this->phone) == '') {
		        $this->_error = addslashes(_WOP_SHOP_REGWARN_PHONE);
		        return false;
		    }
        }
        ...
        if ($config_fields['mobil_phone']['require']){    
            if(trim($this->mobil_phone) == '') {
                $this->_error = addslashes(_WOP_SHOP_REGWARN_MOBIL_PHONE);
                return false;
            }
        }
        ...
        if ($config_fields['fax']['require']){    
            if(trim($this->fax) == '') {
                $this->_error = addslashes(_WOP_SHOP_REGWARN_FAX);
                return false;
            }
        }
        ...
        if ($config_fields['ext_field_1']['require']){
            if(trim($this->ext_field_1) == '') {
                $this->_error = addslashes(_WOP_SHOP_REGWARN_EXT_FIELD_1);
                return false;
            }
        }
        ...
        if ($config_fields['ext_field_2']['require']){
            if(trim($this->ext_field_2) == '') {
                $this->_error = addslashes(_WOP_SHOP_REGWARN_EXT_FIELD_2);
                return false;
            }
        }
        ...
        if ($config_fields['ext_field_3']['require']){
            if(trim($this->ext_field_3) == '') {
                $this->_error = addslashes(_WOP_SHOP_REGWARN_EXT_FIELD_3);
                return false;
            }
        }
			...
			if ($this->delivery_adress) {
                ...
                if ($config_fields['d_title']['require']){
                    if(!intval($this->d_title)) {
                        $this->_error = addslashes(_WOP_SHOP_REGWARN_TITLE_DELIVERY);
                        return false;
                    }
                }
                ...
                if ($config_fields['d_f_name']['require']){
				    if(trim($this->d_f_name) == '') {
					    $this->_error = addslashes(_WOP_SHOP_REGWARN_NAME_DELIVERY);
					    return false;
				    }
                }
                ...
                if ($config_fields['d_l_name']['require']){
				    if(trim($this->d_l_name) == '') {
					    $this->_error = addslashes(_WOP_SHOP_REGWARN_LNAME_DELIVERY);
					    return false;
				    }
                }
				...
				if ($config_fields['d_m_name']['require']){
                    if(trim($this->d_m_name) == '') {
                        $this->_error = addslashes(_WOP_SHOP_REGWARN_MNAME_DELIVERY);
                        return false;
                    }
                }
                ...
                if ($config_fields['d_firma_name']['require']){
                    if(trim($this->d_firma_name) == '') {
                        $this->_error = addslashes(_WOP_SHOP_REGWARN_FIRMA_NAME_DELIVERY);
                        return false;
                    }
                }
                ...
                if (isset($config_fields['d_firma_code']) && $config_fields['d_firma_code']['require']){
                    if(trim($this->d_firma_code) == '') {
                        $this->_error = addslashes(_WOP_SHOP_REGWARN_FIRMA_CODE_DELIVERY);
                        return false;
                    }
                }
                ...
                if (isset($config_fields['d_tax_number']) && $config_fields['d_tax_number']['require']){
                    if(trim($this->d_tax_number) == '') {
                        $this->_error = addslashes(_WOP_SHOP_REGWARN_TAX_NUMBER_DELIVERY);
                        return false;
                    }
                }                
                ...
                if ($config_fields['d_email']['require']){
                    if ( (trim($this->d_email) == "") || !is_email($this->d_email)) {
                        $this->_error = addslashes(_WOP_SHOP_REGWARN_MAIL_DELIVERY);
                        return false;
                    }
                }
				...
				if ($config_fields['d_birthday']['require']){
                    if(trim($this->d_birthday) == '') {
                        $this->_error = addslashes(_WOP_SHOP_REGWARN_BIRTHDAY_DELIVERY);
                        return false;
                    }
                }
                ...
                if ($config_fields['d_home']['require']){
                    if(trim($this->d_home) == '') {
                        $this->_error = addslashes(_WOP_SHOP_REGWARN_HOME_DELIVERY);
                        return false;
                    }
                }
                ...
                if ($config_fields['d_apartment']['require']){
                    if(trim($this->d_apartment) == '') {
                        $this->_error = addslashes(_WOP_SHOP_REGWARN_APARTMENT_DELIVERY);
                        return false;
                    }
                }
                ...
                if ($config_fields['d_street']['require']){
				    if(trim($this->d_street) == '') {
					    $this->_error = addslashes(_WOP_SHOP_REGWARN_STREET_DELIVERY);
					    return false;
				    }
                }
                ...
                if ($config_fields['d_zip']['require']){
                    if (trim($this->d_zip) == ""){
				        $this->_error = addslashes( _WOP_SHOP_REGWARN_ZIP_DELIVERY );
				        return false;
				    }
                }
                ...
                if ($config_fields['d_city']['require']){
                    if (trim($this->d_city) == ''){
                        $this->_error = addslashes( _WOP_SHOP_REGWARN_CITY_DELIVERY );
                        return false;
                    }
                }
                ...
                if ($config_fields['d_state']['require']){
				    if (trim($this->d_state) == ''){
				        $this->_error = addslashes( _WOP_SHOP_REGWARN_STATE_DELIVERY );
				        return false;
				    }
                }
                ...
                if ($config_fields['d_country']['require']){
				    if(!intval($this->d_country)) {
                        $this->_error = addslashes(_WOP_SHOP_REGWARN_COUNTRY_DELIVERY);
                        return false;
                    }
                }                                
                ...
                if ($config_fields['d_phone']['require']){
				    if(trim($this->d_phone) == '') {
				        $this->_error = addslashes(_WOP_SHOP_REGWARN_PHONE_DELIVERY);
				        return false;
				    }
                }
                ...
                if ($config_fields['d_mobil_phone']['require']){    
                    if(trim($this->d_mobil_phone) == '') {
                        $this->_error = addslashes(_WOP_SHOP_REGWARN_MOBIL_PHONE_DELIVERY);
                        return false;
                    }
                }
                ...
                if ($config_fields['d_fax']['require']){    
                    if (trim($this->d_fax) == '') {
                        $this->_error = addslashes(_WOP_SHOP_REGWARN_FAX_DELIVERY);
                        return false;
                    }
                }
                ...
                if ($config_fields['d_ext_field_1']['require']){
                    if(trim($this->d_ext_field_1) == '') {
                        $this->_error = addslashes(_WOP_SHOP_REGWARN_EXT_FIELD_1_DELIVERY);
                        return false;
                    }
                }
                ...
                if ($config_fields['d_ext_field_2']['require']){
                    if(trim($this->d_ext_field_2) == '') {
                        $this->_error = addslashes(_WOP_SHOP_REGWARN_EXT_FIELD_2_DELIVERY);
                        return false;
                    }
                }
                ...
                if ($config_fields['d_ext_field_3']['require']){
                    if(trim($this->d_ext_field_3) == '') {
                        $this->_error = addslashes(_WOP_SHOP_REGWARN_EXT_FIELD_3_DELIVERY);
                        return false;
                    }
                }
