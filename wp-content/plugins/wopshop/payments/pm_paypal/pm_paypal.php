...
class pm_paypal extends PaymentRoot{
	...
	public function showAdminFormParams($params){
        ...
        $ssl_options = array();
        $ssl_options[] = HTML::_('select.option', 4, 'TLSv1_0' , 'id', 'name');
        $ssl_options[] = HTML::_('select.option', 5, 'TLSv1_1' , 'id', 'name');
        $ssl_options[] = HTML::_('select.option', 6, 'TLSv1_2' , 'id', 'name');
	  
        $orders = Factory::getAdminModel('orders'); //admin model
	...
	public function showEndForm($pmconfigs, $order){
        ...
        $item_name = sprintf(_WOP_SHOP_PAYMENT_NUMBER, $order->order_number);
        ...
        $notify_url = SEFLink("controller=checkout&task=step7&act=notify&js_paymentclass=".$pm_method->payment_class."&no_lang=1");
        $return = SEFLink("controller=checkout&task=step7&act=return&js_paymentclass=".$pm_method->payment_class);
        $cancel_return = SEFLink("controller=checkout&task=step7&act=cancel&js_paymentclass=".$pm_method->payment_class);
		
        $_country = Factory::getTable('country');
        ...
        <html>
            ...
            <body>
                <form id="paymentform" action="https://<?php print $host?>/cgi-bin/webscr" name = "paymentform" method = "post">
                    ...
                    <input type='hidden' name='business' value='<?php print $pmconfigs['email_received']; ?>'>        
                    ...
                    <input type='hidden' name='address_override' value='<?php print (int)$pmconfigs['address_override']; ?>'>
                    ...
                    <?php //<input type='hidden' name='bn' value='Wopshop_Cart_ECM'> ?>
                </form>        
                <?php print _WOP_SHOP_REDIRECT_TO_PAYMENT_PAGE?>
                <br>
    ...
    public function getUrlParams($pmconfigs){
        $params = array(); 
        $params['order_id'] =  Request::getInt("custom");
        $params['hash'] = "";
        $params['checkHash'] = 0;
